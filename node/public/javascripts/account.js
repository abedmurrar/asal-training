...
$("#accountForm").on("submit", event => {
    ...
    if (email == '' || !emailRegex.test(email)) {
        ...
        $(".email-msg").html("Invalid email").addClass('error-hint');
    }
    if (isValid) {
        $.ajax({
            url: '/users/',
...
function response(data) {
    if (!data.success) {
        if (data.username) {
            $("#username").addClass('error').removeClass('correct');
            $(".username-msg").html(data.username);
        } else {
            $("#username").addClass('correct');
            $(".username-msg").html('Valid username').addClass('correct-hint').removeClass('error-hint');
        }
        if (data.password) {
            $("#password").addClass('error').removeClass('correct');
            $(".password-msg").html(data.password);
        } else {
            $("#password").addClass('correct');
            $(".password-msg").html('Valid password').addClass('correct-hint').removeClass('error-hint');
        }
        if (data.email) {
            ...
            $(".email-msg").html('Valid email').addClass('correct-hint').removeClass('error-hint');
        }
    }
}